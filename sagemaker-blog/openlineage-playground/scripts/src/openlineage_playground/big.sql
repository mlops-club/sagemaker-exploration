...
/*
CHANGED THE TEMP TABLE NAME FROM _OUT_OF_STOCK_ADS to _OUT_OF_STOCK_ADS_TEMP

snowflake.connector.errors.ProgrammingError: 100132 (P0000): JavaScript execution error: Uncaught Execution of
multiple statements failed on statement "CREATE OR REPLACE TEMP TABLE _..." (at line 346, position 0).

Cannot create temporary object '_OUT_OF_STOCK_ADS' as a permanent object with the same name exists and user does
not have ownership privileges on the permanent object.
*/
CREATE OR REPLACE TEMP TABLE _OUT_OF_STOCK_ADS_TEMP AS
SELECT
    ORDERING_REGION,
    ASIN,
    DATE,
    ...
    OUT_OF_STOCK_14,
    OUT_OF_STOCK_7,
    COALESCE(LAST_VALUE(FORECAST IGNORE NULLS)
      OVER (
...
CREATE OR REPLACE TABLE PATTERN_DB.DATA_SCIENCE_STAGE.TRAINING_DATA AS
SELECT S.*,
